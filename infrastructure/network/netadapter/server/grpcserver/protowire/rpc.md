...
Clients are expected to build RequestMessages and wrap them in SedradMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a SedradMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a SedradMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network sedrad is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which sedra address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether sedrad thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when sedrad is not synced. That is because when sedrad isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known sedrad addresses in the
...
currently connected to this sedrad.
...
| isOutbound | [bool](#bool) |  | Whether this sedrad initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this sedrad |
...
AddPeerRequestMessage adds a peer to sedrad&#39;s outgoing connection list.
This will, in most cases, result in sedrad connecting to said peer.
...
parent chain from some startHash to this sedrad&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this sedrad.
...
ShutDownRequestMessage shuts down this sedrad.
...
This call is only available when this sedrad was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given sedrad addresses
